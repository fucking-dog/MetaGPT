...
import examples.ags.w_action_node.optimized.MATH.graphs.round_13.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        calculation_verification = await self.programmer(problem=f"Verify the following mathematical solution:\n{reviewed_solution['response']}", analysis="Perform necessary calculations to confirm the correctness of the solution.")

        # Final verification and improvement
        verified_solution = await self.custom(input=f"Problem: {problem}\nReviewed solution: {reviewed_solution['response']}\nCalculation verification: {calculation_verification['output']}", instruction=prompt_custom.VERIFY_PROMPT)
