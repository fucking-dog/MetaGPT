...
FINAL_CHECK_AND_CORRECT_PROMPT = """
Perform a final check on the simplified solution and make any necessary corrections. Focus on the following aspects:

1. Ensure the answer directly addresses the question asked in the problem.
2. Check for common mathematical errors such as sign mistakes, calculation errors, or incorrect simplifications.
3. Verify that the answer is in the most appropriate form (e.g., simplified fraction, decimal approximation, or exact value) as required by the problem.
4. Confirm that the LaTeX formatting is correct and the answer is enclosed in \boxed{}.
5. If the problem asks for a specific unit or format, make sure it is included correctly.

If any corrections are needed, make them and provide a brief explanation. If the solution is correct, restate the final answer.

Your response should be concise and focus only on the final, corrected answer. Do not include detailed explanations unless absolutely necessary for clarity.

Important: The final answer must always be formatted using LaTeX notation enclosed in \boxed{}, and it should be the last part of your response.
"""