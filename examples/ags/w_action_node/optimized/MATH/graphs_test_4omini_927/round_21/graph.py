...
import examples.ags.w_action_node.optimized.MATH.graphs.round_21.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        reviewed_solution = await self.custom(input=f"Problem: {problem}\nInitial Solution: {final_solution['response']}", instruction=prompt_custom.REVIEW_AND_REFINE_PROMPT)
        
        # Final formatting step
        formatted_solution = await self.custom(input=f"Problem: {problem}\nReviewed Solution: {reviewed_solution['response']}", instruction=prompt_custom.FORMAT_PROMPT)
