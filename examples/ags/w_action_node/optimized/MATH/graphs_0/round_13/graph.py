...
from examples.ags.w_action_node.optimized.Gsm8K.graphs.round_13.prompt import *
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        think = await self.custom(input=problem+rephrased_problem['rephrased_problem'], instruction=THINK_PROMPT)
        self_ask = await self.custom(input=problem+rephrased_problem['rephrased_problem']+think['response'], instruction=SELF_ASK_PROMPT)
        solutions = []
        for _ in range(3):  # Generate 3 solutions
            solution = await self.generate(problem=problem+rephrased_problem['rephrased_problem']+think['response']+self_ask['response'])
