...
import examples.ags.w_action_node.optimized.MATH.graphs.round_15.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        factorization = await self.custom(input=problem, instruction=prompt_custom.PRIME_FACTORIZATION_PROMPT)
        
        code_solutions = []
        for _ in range(3):  # Generate 3 solutions
            code_solution = await self.programmer(problem=problem + f"\nPrime factorization: {factorization['response']}")
        ...
        refined_solution = await self.custom(input=problem + f"\nEnsemble result:\n{ensemble_result['response']}\nPrime factorization: {factorization['response']}", instruction=prompt_custom.REFINE_SOLUTION_PROMPT)
