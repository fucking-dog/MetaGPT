...
import examples.ags.w_action_node.optimized.MATH.graphs.round_27.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        solution_approaches = await self.custom(input=problem + f"\nEnsemble result:\n{ensemble_result['response']}\nPrime factors: {prime_factors['response']}", instruction=prompt_custom.GENERATE_SOLUTION_APPROACHES)
        
        refined_solution = await self.custom(input=problem + f"\nEnsemble result:\n{ensemble_result['response']}\nPrime factors: {prime_factors['response']}\nSolution approaches:\n{solution_approaches['response']}", instruction=prompt_custom.REFINE_SOLUTION_PROMPT)
        
        # Error checking and correction
        final_solution = await self.custom(input=problem + f"\nRefined solution:\n{refined_solution['response']}", instruction=prompt_custom.ERROR_CHECK_PROMPT)
