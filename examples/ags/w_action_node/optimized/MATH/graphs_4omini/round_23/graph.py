...
import examples.ags.w_action_node.optimized.MATH.graphs.round_23.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        # Add mathematical review step
        math_reviewed_solution = await self.custom(input=problem + f"\nVerified solution:\n{verified_solution['response']}", instruction=prompt_custom.MATH_REVIEW_PROMPT)
        
        # Add final simplification and formatting check
        final_solution = await self.custom(input=problem + f"\nMath reviewed solution:\n{math_reviewed_solution['response']}", instruction=prompt_custom.SIMPLIFY_AND_FORMAT_PROMPT)
