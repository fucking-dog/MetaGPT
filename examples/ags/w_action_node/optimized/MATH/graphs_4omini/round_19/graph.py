...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        # Add error analysis and correction step
        error_analyzed_solution = await self.custom(input=problem + f"\nVerified solution:\n{verified_solution['response']}", instruction=prompt_custom.ERROR_ANALYSIS_PROMPT)
        
        final_solution = await self.custom(input=problem + f"\nError analyzed solution:\n{error_analyzed_solution['response']}", instruction=prompt_custom.SIMPLIFY_AND_FORMAT_PROMPT)
