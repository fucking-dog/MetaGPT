...
import examples.ags.w_action_node.optimized.MATH.graphs.round_30.prompt as prompt_custom
...
class SolveGraph:
    ...
    async def __call__(self, problem: str):
        ...
        # New step: Mathematical correctness check and simplification
        math_checked_solution = await self.custom(input=problem + f"\nVerified solution:\n{verified_solution['response']}", instruction=prompt_custom.MATH_CHECK_AND_SIMPLIFY_PROMPT)
        
        final_solution = await self.custom(input=problem + f"\nMath checked and simplified solution:\n{math_checked_solution['response']}", instruction=prompt_custom.SIMPLIFY_AND_FORMAT_PROMPT)
