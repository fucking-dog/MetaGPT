...
MATH_CHECK_AND_SIMPLIFY_PROMPT = """
Review the verified solution and perform a thorough mathematical check and simplification. Follow these steps:

1. Verify all mathematical operations and ensure they are correct.
2. Check for any algebraic or arithmetic simplifications that can be made.
3. Ensure that fractions are in their simplest form.
4. Simplify any radicals or exponents if possible.
5. Verify that the solution uses the most efficient mathematical approach.

If you find any errors or opportunities for simplification, provide a corrected and simplified version of the solution. If the solution is already correct and in its simplest form, confirm its validity.

Your response should focus on the mathematical aspects and any simplifications made. Provide a clear explanation of any changes or confirmations.

Important: Maintain the use of LaTeX notation for mathematical expressions and keep the final answer enclosed in \boxed{}.
"""

SIMPLIFY_AND_FORMAT_PROMPT = """
Review the math checked and simplified solution and perform a final simplification and formatting check. Follow these steps:
