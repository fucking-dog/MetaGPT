    ...
    async def single_run(self, problem: Dict, types: Dict) -> Dict:
        ...
        for index, phase in enumerate(resolver_plan["plan"]):
            if phase["phase"] == "inference":
                answer = self.inference(problem["description"], current_trajectory, subgoal=phase["desc"])
                current_trajectory.append({"plan":phase["desc"],"reason":phase["reason"],"answer":answer})
            elif phase["phase"] == "di":
                answer = self.di_run(problem["description"], current_trajectory, subgoal=phase["desc"])
                current_trajectory.append({"plan":phase["desc"],"reason":phase["reason"],"answer":answer})
            elif phase["phase"] == "logic_validate":
                result = self.logic_validate(problem["description"], current_trajectory, subgoal=phase["desc"])
                if result["judge"]:
        ...
        if self.result_validate(problem["description"], current_trajectory):
            pass
        else:
            current_trajectory.append(self.inference_final(problem["description"], current_trajectory))
        return {"current_trajectory": current_trajectory}
    
    
    async def di_run(self, problem, current_trajectory, subgoal):
        DI = DataInterpreter()
        record = await DI.run(di_prompt.format(problem_desc=problem, subgoal=subgoal))
        return record
